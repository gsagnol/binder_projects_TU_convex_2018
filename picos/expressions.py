...
class AffinExp(Expression):
    ...
    **Overloaded operators**
    ...
    def fromScalar(cls, scalar):
        const, string = _retrieve_matrix(scalar, (1, 1))
        return cls(constant = const, string = string)

    @classmethod
    def fromMatrix(cls, matrix, size = None):
        const, string = _retrieve_matrix(matrix, size)
    ...
    def sparse_rows(
        ...
        :param bool lowerTriangle: Whether to return only the upper triangular
            part of the expression.
        :param function indexFunction: Instead of adding the local variable
            index to the value returned by varOffsetMap, use the return value of
            this function, that takes as argument the variable and its local
            index, as the "column index", which need not be an integer. When
            this parameter is passed, the parameter varOffsetMap is ignored.
...
class Variable(AffinExp):
    ...
    def set_value(self, value, index = None):
        value, _ = _retrieve_matrix(value, self.size)
        n, m = self.size
        ...
        if self.vtype is "symmetric":
            ...
            if value.size == self.size:
                ...
                assert value.size[1] is 1, \
                    "_retrieve_matrix was supposed to return a column vector."
            else:
                raise TypeError(
