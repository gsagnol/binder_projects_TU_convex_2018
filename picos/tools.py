...
# XXX: This __all__ expression is hard to maintain. At the time of writing this,
#      the only names defined in this file but not included in __all__ are:
#      '_copy_dictexp_to_new_vars', 'allIdent', 'cut_in_frames', 'drawGraph',
#      'findEndOfInd', 'flow_Constraint', 'is_index_char', 'lse', and
#      'put_indices_on_frames'. It does not appear that these names need to be
#      hidden when compared to the names that are exported. Furthermore, some
#      names beginning with "_" are exported, which breaks the convention that
#      "from … import *" does not import these names.
# TODO: Consider removing the __all__ statement and hide exports via a leading
#       underscore as necessary.
__all__ = ['INTEGER_TYPES',
           '_retrieve_matrix',
           '_svecm1_identity',
           'eval_dict',
           'detect_range',
...
def simplex(gamma=1):
    """returns a :class:`TruncatedSimplex <picos.expression.TruncatedSimplex>` object representing the set :math:`\{x\geq 0: ||x||_1 \leq \gamma \}`.
...
def truncated_simplex(gamma=1, sym=False):
    """returns a :class:`TruncatedSimplex <picos.expression.TruncatedSimplex>` object representing the set:
...
def expcone():
    """returns a :class:`ExponentialCone <picos.expression.ExponentialCone>` object representing the set :math:`\operatorname{closure}\ \{(x,y,z): y>0, y\exp(x/y)\leq z \}`
...
def sumexp(x, y = None):
    """
    A shorthand for :class:`SumExponential <picos.SumExponential>`.
...
def kullback_leibler(x, y = None):
    """
    A shorthand for :class:`KullbackLeibler <picos.KullbackLeibler>`.
...
def logsumexp(exp):
    """
    A shorthand for :class:`LogSumExp <picos.LogSumExp>`.
...
def _retrieve_matrix(mat, exSize=None):
    ...
    **Example:**
    ...
    if exSize is not None and not _is_integer(exSize) \
...
# XXX: Implement glyphs from here.
...
def _svecm1_identity_factor(factors, variable):
    """
    ...
    identity = _svecm1_identity(variable.vtype, variable.size)
...
def new_param(name, value):
    ...
    **Example:**
    ...
    if isinstance(value, list):
        ...
        and  all([_is_realvalued(xi) for x in value for xi in x]):
...
def _copy_exp_to_new_vars(exp, cvars, complex=None):
    ...
    if isinstance(exp, Variable):
        ...
        newexp = _copy_exp_to_new_vars(exp.Exp, cvars, complex=complex)
        newexp2 = _copy_exp_to_new_vars(exp.Exp2, cvars, complex=complex)
...
def flow_Constraint(
    ...
    :param flow_value: The value of the flow, or a list of values in case of a
        ...
        :class:`affine expressions <picos.AffinExp>`.
