...
class Problem(object):
    ...
    def set_objective(self, typ, expr):
        ...
        :type expr: :class:`Expression <picos.Expression>`
        """
        if typ == 'find':
    ...
    def set_var_value(self, name, value, optimalvar=False):
        """
        Sets the :attr:`value <picos.expressions.Variable.value>` of the given
        variable.

        :param str name: Name of the variable.
        :type name: Either a string name or a (name, index) tuple for multiple
            solutions.

        :param value: The value to be set.
        :type value: Anything recognized by :func:`_retrieve_matrix
            <picos.tools._retrieve_matrix>`
    ...
    def set_option(self, key, val):
        ...
        * General options common to all solvers:
          ...
          * ``strict_options = False`` -- If ``True``, unsupported general
            ...
            :class:`UnsupportedOptionError <picos.solver.UnsupportedOptionError>`
          ...
          * ``noprimals = False`` -- If ``True``, do not copy the optimal
            variable values in the :attr:`value<picos.Variable.value>` attribute
          ...
          * ``hotstart = False`` -- If `True`, tells the mixed integer optimizer
            to start from the (partial) solution specified in the
            :attr:`value <picos.Variable.value>` attributes of the variables.
        ...
        * Specific options available for CPLEX:
          ...
          * ``cplex_params = {}`` -- A dictionary of
            ...
            to be set before the CPLEX optimizer is called.
    ...
    def add_variable(self, name, size=1, vtype='continuous', lower=None,
        ...
        :param vtype: :attr:`Type <picos.Variable.vtype>` of the variable.
            Can be any of
    ...
    def copy(self):
        ...
        newObjective = _copy_exp_to_new_vars(self.objective[1], newVars)
        ...
        newProblem._options = _NonWritableDict(self.options)
    ...
    def add_constraint(self, constraint, key=None):
        ...
        :type constraint: :class:`Constraint <picos.Constraint>`
        :param str key: Optional name of the constraint.

        :returns: The constraint that was added to the problem, which may be a
            :class:`MetaConstraint <picos.MetaConstraint>` that contains further
    ...
    def add_list_of_constraints(self, lst, it=None, indices=None, key=None):
        ...
        :param list lst: A list of :class:`constraints <picos.Constraint>`.
        :param it: DEPRECATED
    ...
    def get_variable(self, name):
        ...
        :returns: A :class:`PICOS variable <picos.Variable>`, if ``name``
            refers to a single variable, or a list or a dictionary thereof, if
            the collection of variables specified by ``name`` is a list or a
            dictionary, respectively.
        """
    ...
    def get_constraint(self, ind):
        ...
        :returns: A :class:`constraint <picos.Constraint>` or a list thereof.
    ...
    def minimize(self, obj, **options):
        ...
        :type obj: :class:`Expression <picos.Expression>`
    ...
    def solve(self, **options):
        ...
        by querying the :attr:`value <picos.Expression.value>` property of the
        variables and the optimal dual values can be accessed via the
        :attr:`dual <picos.Constraint.dual>` property of the constraints.
        ...
        for c in self.constraints:
            if isinstance(c, LMIConstraint):
                ...
                if c.lhs.is_real() and c.rhs.is_real():
                    ...
                    lhs = _copy_exp_to_new_vars(c.lhs, cvars, complex=False)
                    rhs = _copy_exp_to_new_vars(c.rhs, cvars, complex=False)
                elif c.is_equality():
                    # At least one side is not real and we have an equality, so
                    # equate both the real and the imaginary part.
                    lhs = _copy_exp_to_new_vars(c.lhs, cvars, complex=True)
                    rhs = _copy_exp_to_new_vars(c.rhs, cvars, complex=True)
                else:
                ...
                if c.ne.is_real():
                    ne = _copy_exp_to_new_vars(c.ne, cvars, complex=False)
                else:
                    ne = _copy_exp_to_new_vars(c.ne, cvars, complex=True)

                if isinstance(c, SOCConstraint):
                    if not c.ub.is_real():
                        raise Exception('Found a complex expression in the RHS '
                            'of a nonlinear constraint.')
                    ub = _copy_exp_to_new_vars(c.ub, cvars, complex=False)
                    realConstraint = SOCConstraint(ne, ub)
                else: # RSOCConstraint
                    if not (c.ub1.is_real() and c.ub2.is_real()):
                        raise Exception('Found a complex expression in the RHS '
                            'of a nonlinear constraint.')
                    ub1 = _copy_exp_to_new_vars(c.ub1, cvars, complex=False)
                    ub2 = _copy_exp_to_new_vars(c.ub2, cvars, complex=False)
                    realConstraint = RSOCConstraint(ne, ub1, ub2)

                real.add_constraint(realConstraint, c.name)
            elif isinstance(c, QuadConstraint) or isinstance(c, LSEConstraint):
                if c.le0.is_real():
                    le0 = _copy_exp_to_new_vars(c.le0, cvars, complex=False)
                else:
                    le0 = _copy_exp_to_new_vars(c.le0, cvars, complex=True)
