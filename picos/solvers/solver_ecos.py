...
class ECOSSolver(Solver):
    ...
    def stack(self, *args):
        ...
        if isinstance(args[0], ecos.sparse.base.spmatrix):
            ...
            #HACK
            #OLD return ecos.sparse.vstack(args, format = "csc")
            args_n0 = [argi for argi in args if argi.shape[0]>0]
            if args_n0:
                return ecos.sparse.vstack(args_n0, format = "csc")
            else:
                return ecos.sparse.csc_matrix(args[0])
            #->HACK
